...
class TokenSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.decimals <= MAX_TOKEN_DECIMALS:
            raise ValueError(f"Token {self.symbol}: decimals must be 0-{MAX_TOKEN_DECIMALS}, got {self.decimals}")
        if self.price_usd < 0:
            raise ValueError(f"Token {self.symbol}: price_usd cannot be negative, got {self.price_usd}")
