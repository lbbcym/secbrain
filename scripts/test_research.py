...
async def test_rate_limiting():
    ...
    print(f"[OK] Made 12 calls in {elapsed:.2f}s")
    print(f"[OK] All calls completed: {len(results) == 12}")
    print(f"[OK] Dry-run responses: {all('DRY-RUN' in r.get('answer', '') for r in results)}")
...
async def test_ttl_caching():
    ...
    print(f"[OK] First call cached: {result1.get('cached', False)} (expected: False)")
    print(f"[OK] Second call cached: {result2.get('cached', False)} (expected: True)")
    print(f"[OK] Cache age: {result2.get('cache_age_hours', 0):.4f} hours")
...
async def test_specialized_methods():
    ...
    print(f"[OK] research_severity_context: {len(severity_result.get('answer', ''))} chars")
    ...
    print(f"[OK] research_attack_vectors: {len(attack_result.get('answer', ''))} chars")
    ...
    print(f"[OK] research_market_conditions: {len(market_result.get('answer', ''))} chars")
...
async def test_backward_compatibility():
    ...
    print(f"[OK] research_technology: {len(tech_result.get('answer', ''))} chars")
    ...
    print(f"[OK] research_endpoint: {len(endpoint_result.get('answer', ''))} chars")

    cwe_result = await research.research_cwe(
        cwe_id="CWE-862",
        run_context=run_context,
    )
    print(f"[OK] research_cwe: {len(cwe_result.get('answer', ''))} chars")
    ...
    print(f"[OK] research_writeups: {len(writeup_result.get('answer', ''))} chars")
...
async def test_call_limits():
    ...
    print(f"[OK] Limited responses: {limited_count}/5 (expected: 2)")
    print(f"[OK] Call count: {research._call_count} (expected: 3)")
